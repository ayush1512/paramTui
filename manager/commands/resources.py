...
def resource_gpu_usage(ssh_conn):
    ...
    output = ssh_conn.execute_command("nvidia-smi 2>/dev/null || echo 'No GPU available or nvidia-smi not found'")
    if output:
...
def resource_node_availability(ssh_conn):
    ...
    output = ssh_conn.execute_command("sinfo -o '%20P %5a %10l %6D %8t %N' 2>/dev/null")
    if output:
...
def quota_disk_usage(ssh_conn):
    ...
    output = ssh_conn.execute_command("quota -s 2>/dev/null || lfs quota -h ~ 2>/dev/null || df -h ~")
    if output:
