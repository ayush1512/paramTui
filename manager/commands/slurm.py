...
def job_get_running(ssh_conn):
    ...
    output = ssh_conn.execute_command("squeue -u $USER -t RUNNING -o '%.18i %.15j %.8T %.10M %.9l %.6D %.4C %.10m %R' 2>/dev/null")
    if output:
...
def job_get_pending(ssh_conn):
    ...
    output = ssh_conn.execute_command("squeue -u $USER -t PENDING -o '%.18i %.15j %.8T %.10M %.9l %.6D %.4C %R' 2>/dev/null")
    if output:
...
def job_get_completed(ssh_conn, days=7):
    ...
    output = ssh_conn.execute_command(f"sacct -u $USER -S {start_date} --state=COMPLETED -o JobID,JobName,State,Elapsed,MaxRSS,ExitCode -n 2>/dev/null | head -30")
    if output:
...
def job_get_failed(ssh_conn, days=7):
    ...
    start_date = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
    output = ssh_conn.execute_command(f"sacct -u $USER -S {start_date} --state=FAILED,CANCELLED,TIMEOUT -o JobID,JobName,State,Elapsed,ExitCode -n 2>/dev/null | head -30")
    if output:
...
def job_resource_usage(ssh_conn, job_id):
    ...
    output = ssh_conn.execute_command(f"seff {job_id} 2>/dev/null || sacct -j {job_id} -o JobID,Elapsed,MaxRSS,MaxVMSize,AveRSS,AveCPU -n")
    if output:
...
def job_view_log(ssh_conn, job_id, log_type="out"):
    ...
    output = ssh_conn.execute_command(f"find ~ -name '*{job_id}*.{log_type}' -type f 2>/dev/null | head -1")
